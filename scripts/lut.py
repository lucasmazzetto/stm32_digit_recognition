...
def build_u8_to_q16_lut(frac_bits: int = 16) -> np.ndarray:
    """
    @brief Builds a 256-entry LUT that matches host normalization -> FP16 -> Q-format.

    Mapping per entry v in [0, 255]:
      1) normalized = (v / 255 - 0.5) / 0.5
      2) fp16_value = float16(normalized)
      3) q_value = round(fp16_value * 2^frac_bits)
...
def write_source_file(path: Path, header_name: str, lut_symbol: str,
    ...
    with path.open("w", encoding="utf-8") as file:
        ...
        file.write(
            f"const int32_t {lut_symbol}[{size_symbol}] = {{{values_text}}};\n"
        )


def generate_c_lut_files(output_dir: Path, frac_bits: int = 16) -> tuple[Path, Path]:
    """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate LUT C files (lut.h and lut.c) for firmware preprocessing.",
